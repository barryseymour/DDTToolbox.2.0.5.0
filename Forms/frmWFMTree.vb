...
Public Class frmWFMTree

#Region "Public Variables & constants"
    Public Shared CopyNode As WFMTreeNode
...
#Region "Private Variables & Constants"
    ...
    Private WFMTreeViewBuilt As Boolean
    Private CurrDPVersion As Decimal
    Private DraggedNode As New WFMTreeNode
    Private SaveNode As New WFMTreeNode
    ...
    Private NewNode As WFMTreeNode
...
#End Region
    ...
    Private Sub frmWFMTreeTabs_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs)
        If TreeDirty Then
            If e.CloseReason = CloseReason.UserClosing Then
                Dim aResponse As DialogResult = RichMessageBoxShow("Are you sure you want to close the WFM Tree toolbox window without saving changes?", _
            ...
            Else
                ...
                If EditingAllowed Then
                    Dim aResponse As DialogResult = RichMessageBoxShow("Do you want to save the pending changes to the WFM Tree?", _
                    ...
                    If aResponse = System.Windows.Forms.DialogResult.Yes Then
                        SaveCurrentTreeNode(DirectCast(tvwWFM.SelectedNode, WFMTreeNode))
    ...
    Private Sub BuildReportsTable()
        BuildDataSet(sqlConnection, "exec sToolbox_GetReportsInfo 0, 2", "Reports", dsroot)
        BuildDataSet(sqlConnection, "exec sToolbox_GetReportsInfo 1, 2", "ReportsComBoBox", dsroot)
    ...
    Private Sub frmWFMTree_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        For Each dr As DataRow In dsroot.Tables("Reports").Rows
            ...
            i += 1
        Next
        BuildDataSet(sqlConnection, "exec sToolbox_GetFoldersInfo 2", "Folders", dsroot)
        ...
        BuildDataSet(sqlConnection, "exec dbo.sBuildTree 2", "sBuildTree", dsroot)
        ...
        HandleControls()
        BuildControlsTab(DirectCast(tvwWFM.TopNode, WFMTreeNode).TreeLevelID)
    ...
    Private Function ReportIdInTree(ByVal ReportID As Integer, ByVal aTreeView As TreeView, Optional ByRef theNodes As TreeNodeCollection = Nothing) As Boolean
        ...
        For Each aNode As WFMTreeNode In theNodes
    ...
    Private Sub RefreshUsedReportNumList()
        ...
        End If
        BuildDataSet(sqlConnection, "SELECT ReportNumber, ReportID FROM dbo.trReports WHERE ApplicationID = 2 ORDER BY ReportNumber", "UsedReportNumbers", dsroot)
        ...
        For Each urR In usedReportsDR
            ...
            If ReportIdInTree(RepID, tvwWFM) = False Then
    ...
    Private Sub BuildTreeView()
        BuildTree_ToolBox(tvwWFM, TreeImageList, 2)
        InitialBuild = False
        TreeDirty = False
        '--- Needed to select 1st node in tree ---
        tvwWFM.SelectedNode = tvwWFM.TopNode
    End Sub
    Private Sub BuildTree_ToolBox(ByVal tvw As TreeView, ByVal img As DDT_ImageList, Optional ByVal ApplicationID As Integer = 1)
        BuildDataSet(sqlConnection, "dbo.sBuildTree " & ApplicationID, "WFMTree", dsroot)
        Dim drdata As DataRow() = dsroot.Tables("WFMTree").Select("TreeLevelID_Parent=-1")
        Dim r As DataRow
        CallFromType = "Sub"
        CallFrom = "BuildTree"
            ...
            With tvw
                ...
                If Not drdata.Length <= 0 Then
                    For Each r In drdata
                        ...
                        Dim myNode As WFMTreeNode = GetTreeNodeDetails_ToolBox(r)
                        With myNode
        ...
        End Try
    End Sub
    Public Function GetTreeNodeDetails_ToolBox(ByVal rowItem As DataRow) As WFMTreeNode
        CallFromType = "Function"
        CallFrom = "GetTreeNodeDetails"

        Try
            Dim myNode As New WFMTreeNode
            With myNode
    ...
    Public Sub AddNodes_ToolBox(ByVal ParentNode As TreeNode, ByVal iParentID As Integer)
        Dim drdata As DataRow() = dsroot.Tables("WFMTree").Select("TreeLevelID_Parent=" & iParentID)
        Dim r As DataRow
    ...
    Public Sub AddNodes_ToolBox(ByVal ParentNode As TreeNode, ByVal iParentFolderID As Integer, ByVal stype As String)
        ...
        Try
            If Not drData.Length <= 0 Then
                For Each r In drData
                    Dim iNodeID As Integer = r("TreeLevelID")
                    Dim myNode As WFMTreeNode = GetTreeNodeDetails(r)
                    With myNode
                        .ImageIndex = r("ImageID") - 1
                        .Text = r("NodeName")
                        .Name = r("NodeID")
                    ...
                    End With
                    If stype = "F" Then
                        If r("ItemType") = "F" Then ParentNode.Nodes.Add(myNode) 'LAshby - new code to replacee line above
                    Else
                        ParentNode.Nodes.Add(myNode)  'LAshby - new code to replacee line above
                    End If
    ...
    Private Sub HandleControls()
        Try
            If tvwWFM.Nodes.Count > 0 Then

                Dim thisNode As WFMTreeNode = tvwWFM.SelectedNode
                Dim nodeLvlCount As Integer = 0
                If tvwWFM.SelectedNode.Parent Is Nothing Then
                    nodeLvlCount = tvwWFM.Nodes.Count
                Else
                    nodeLvlCount = tvwWFM.SelectedNode.Parent.Nodes.Count
                End If
                ...
                btnMoveUp.Enabled = EditingAllowed And (editmode = notEditing) And Not tvwWFM.SelectedNode Is Nothing _
                    And Not tvwWFM.SelectedNode.Index = 0
                cmTreeNodeOperations.Items("MoveUpToolStripMenuItem").Enabled = btnMoveUp.Enabled

                Dim nodeCount As Integer
                If Me.tvwWFM.SelectedNode.Parent Is Nothing Then
                    nodeCount = tvwWFM.Nodes.Count
                Else
                    nodeCount = tvwWFM.SelectedNode.Parent.Nodes.Count
                End If

                tvwWFM.Enabled = (editmode = notEditing)
                dgvTreeProperties.Enabled = True
                btnMoveDown.Enabled = EditingAllowed And (editmode = notEditing) And Not tvwWFM.SelectedNode Is Nothing And tvwWFM.SelectedNode.Index < nodeLvlCount - 1
                ...
                btnCopyItem.Enabled = EditingAllowed And ((Not tvwWFM.SelectedNode Is Nothing And editmode = notEditing And Strings.Left(DirectCast(tvwWFM.SelectedNode, WFMTreeNode).ItemType, 6).ToUpper <> "FOLDER") And Not UserEdits_NodesMoved)
                ...
                btnDelete.Enabled = EditingAllowed And (Not tvwWFM.SelectedNode Is Nothing And editmode = notEditing) 'And chkAllowDelete.Checked
                ...
                btnEdit.Enabled = EditingAllowed And (Not tvwWFM.SelectedNode Is Nothing And editmode = notEditing And Not UserEdits_NodesMoved)
                ...
                btnControlADD.Enabled = (editmode <> notEditing) And DirectCast(tvwWFM.SelectedNode, WFMTreeNode).ItemType = "R" And tvwWFM.SelectedNode.Text <> "NEW ITEM"
                btnControlEdit.Enabled = (editmode <> notEditing) And DirectCast(tvwWFM.SelectedNode, WFMTreeNode).ItemType = "R" And tvwWFM.SelectedNode.Text <> "NEW ITEM" And dgvReportControls.SelectedRows.Count > 0
                btnControlDelete.Enabled = (editmode <> notEditing) And DirectCast(tvwWFM.SelectedNode, WFMTreeNode).ItemType = "R" And tvwWFM.SelectedNode.Text <> "NEW ITEM" And dgvReportControls.SelectedRows.Count > 0
                ...
                If tvwWFM.SelectedNode.Text = "NEW ITEM" Then
    ...
    Private Sub tvwWFM_AfterSelect(ByVal sender As System.Object, ByVal e As System.Windows.Forms.TreeViewEventArgs) Handles tvwWFM.AfterSelect, tvwWFM.AfterSelect
        Dim myNode As WFMTreeNode = DirectCast(tvwWFM.SelectedNode, WFMTreeNode)        '--> Cast TreeNode to WFMTreeNode
    ...
    Private Sub HandleNodeClick(ByVal myNode As WFMTreeNode)
        ...
        chkSchedulingEnabled.Checked = False 'scheduling not enabled in WFM
        ...
        BuildControlsTab(DirectCast(tvwWFM.SelectedNode, WFMTreeNode).TreeLevelID)
        ...
        BuildReportsViewTab(DirectCast(tvwWFM.SelectedNode, WFMTreeNode).ReportID)
    End Sub
    ...
    Private Sub frmWFMTree_Resize(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Resize
        ResizeControls()
    End Sub
    ...
    Private Sub btnOpenAllFolders_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnOpenAllFolders.Click
        ...
        tvwWFM.ExpandAll()
        tvwWFM.EndUpdate()
        tvwWFM.SelectedNode = tvwWFM.Nodes(0)
        tvwWFM.SelectedNode.EnsureVisible()
    End Sub
    ...
    Private Sub DisplayProperties(ByVal myNode As WFMTreeNode)
        Dim i As Integer
    ...
    Private Sub dgvUsedReportNumbers_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvUsedReportNumbers.CellDoubleClick
        If editmode <> editing Then
            Dim aNode As WFMTreeNode = WFMGetNodeByKey(tvwWFM, "ReportID=" + dgvUsedReportNumbers.Item("ReportID", dgvUsedReportNumbers.CurrentRow.Index).Value.ToString, True)
        End If
    End Sub

    Private Sub frmWFMTree_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        ResizeControls()
    End Sub
    ...
    Private Sub SetTreeDataState(ByVal state As String)
        If state = "dirty" Then
            tvwWFM.BackColor = Color.LightSteelBlue
            TreeDirty = True
            Dim user As String = Strings.Right(My.User.Name, Len(My.User.Name) - InStrRev(My.User.Name, "\"))
            Dim sql As String = "exec DDT_Common.dbo.sToolbox_UpdateTreeLevels_EditStatus '" + user + "', 2"
            Try
            ...
            End Try
        Else
            tvwWFM.BackColor = Color.White
            TreeDirty = False
            Dim sql As String = "exec DDT_Common.dbo.sToolbox_UpdateTreeLevels_EditStatus Null, 2"
            Try
                sqlCmd.CommandType = CommandType.Text
                sqlCmd.CommandText = sql
                sqlCmd.Connection = sqlConnection
    ...
    Private Sub EditNotAllowedMessage()
        MessageBox.Show("Editing of the WFM Tree is not currently allowed because it is being modified by " + LockedForEditingBy + ".", "Changes not allowed", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
        editmode = notEditing
        HandleControls()
    End Sub
    ...
    Private Sub CheckEditingStatus()
        ...
        Dim sql As String = "exec DDT_Common.dbo.sToolbox_GetTreeLevels_EditStatus 2"
        sqlConnection = New System.Data.SqlClient.SqlConnection(connStr)
    ...
    Private Sub EditingReenabled()
        MessageBox.Show("Editing of the WFM Tree has been reenabled. The WFM Tree will be reloaded after pressing OK.", "Editing permitted", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
        Me.Update()
        InitialBuild = True
        BuildTreeView()
        RefreshUsedReportNumList()
    ...
    Private Sub btnRefresh_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnRefresh.Click
        Dim aNode As WFMTreeNode
        Dim RepBtnSelected As Boolean = rbReport.Checked
        Dim SaveSelectedIndex = cboItemTypeSelect.SelectedIndex
        aNode = tvwWFM.SelectedNode
        BuildTreeView()
        RefreshUsedReportNumList()
        BuildReportsTable()
        WFMGetNodeByKey(tvwWFM, "TreeLevelID=" + aNode.TreeLevelID.ToString, True)
    End Sub
            ...
            If UserEdits_TREE Or UserEdits_NodesMoved Or UserEdits_CONTROL Then
                ...
                Else
                    If MessageBoxResult = Windows.Forms.DialogResult.OK Or UserEdits_NodesMoved Or UserEdits_CONTROL Then
                        Me.Cursor = Cursors.WaitCursor
                        If SaveNode.Text = "" Or UserEdits_NodesMoved Then SaveNode = tvwWFM.SelectedNode
                        editmode = notEditing
                        BuildTreeView()

                        WFMGetNodeByKey(tvwWFM, "TreeLevelID=" + SaveNode.TreeLevelID.ToString, True)
                        BuildControlsTab(DirectCast(tvwWFM.SelectedNode, WFMTreeNode).TreeLevelID)
                        BuildReportsViewTab(DirectCast(tvwWFM.SelectedNode, WFMTreeNode).ReportID)
                        UserEdits_TREE = False
                        UserEdits_CONTROL = False
    ...
    Private Sub SaveCurrentTreeNode(ByVal NodeToSave As WFMTreeNode)
        CheckEditingStatus()
        Dim tmpCurrNode As WFMTreeNode = NodeToSave
        If EditingAllowed Then
            'mburts
            ...
            With DirectCast(tvwWFM.SelectedNode, WFMTreeNode)
                SqlStr = "exec dbo.sToolbox_UpdateTreeLevels "
                SqlStr += GetReportPropertyValue("TreeLevelID") + ","
                If DirectCast(tvwWFM.SelectedNode, WFMTreeNode).TreeLevelID_Parent.ToString = Nothing Then
                    SqlStr += "0" + ","
                Else
                    SqlStr += DirectCast(tvwWFM.SelectedNode, WFMTreeNode).TreeLevelID_Parent.ToString + ","
                End If
                ...
                SqlStr += "0" + "," 'scheduling never enabled for WFM
                If .PrevNode Is Nothing Then
                    SqlStr += "0" + ","
                Else
                    SqlStr += DirectCast(.PrevNode, WFMTreeNode).TreeLevelID.ToString + ","
                End If
                SqlStr += "1"
            End With
            ...
            Call WFMGetNodeByKey(tvwWFM, "TreeLevelID=" + ResultTreeLevelID.ToString, True)
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        SaveCurrentTreeNode(tvwWFM.SelectedNode)
    End Sub
    ...
    Private Sub tvwWFM_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles tvwWFM.DoubleClick, tvwWFM.DoubleClick
        If DirectCast(tvwWFM.SelectedNode, WFMTreeNode).ItemType = "R" Then
            EditRecord()
        End If
    End Sub
    ...
    Private Sub btnCopyItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCopyItem.Click
        Try
            CopyNode = DirectCast(tvwWFM.SelectedNode, WFMTreeNode)
...
"select *, CM.ControlName from dbo.trControls C (nolock) inner join dbo.trControls_Master CM (nolock) ON C.Controls_MasterID = CM.Controls_MasterID Order By ControlTitle", "trControls", dsroot)
        ...
        With frmWFMEditReportControls
            .txtReportControlID.ReadOnly = True
            .txtApplicationID.Text = 2
            .txtApplicationID.ReadOnly = True
            .txtReportID.Text = DirectCast(tvwWFM.SelectedNode, WFMTreeNode).ReportID
            .txtReportID.ReadOnly = True
            .txtTreeLevelID.Text = DirectCast(tvwWFM.SelectedNode, WFMTreeNode).TreeLevelID
        ...
        Dim result As Integer = frmWFMEditReportControls.ShowDialog
        ...
        If result = DialogResult.OK Then
            ...
            With dsroot.Tables("ReportControls")
                ...
                .Rows(rowIndex).Item("Report_ControlID") = frmWFMEditReportControls.txtReportControlID.Text.ToString
                .Rows(rowIndex).Item("ApplicationID") = frmWFMEditReportControls.txtApplicationID.Text.ToString
                .Rows(rowIndex).Item("ReportID") = frmWFMEditReportControls.txtReportID.Text
                .Rows(rowIndex).Item("TreeLevelID") = frmWFMEditReportControls.txtTreeLevelID.Text
                .Rows(rowIndex).Item("ControlsID") = frmWFMEditReportControls.cboControlsID.SelectedValue
                .Rows(rowIndex).Item("ControlTitle") = frmWFMEditReportControls.txtControlTitle.Text
                .Rows(rowIndex).Item("ControlName") = frmWFMEditReportControls.txtControlName.Text
                .Rows(rowIndex).Item("IndexOrder") = IIf(frmWFMEditReportControls.txtIndexOrder.Text = "", 0, frmWFMEditReportControls.txtIndexOrder.Text)
                .Rows(rowIndex).Item("TabIndex") = IIf(frmWFMEditReportControls.cboTabIndex.Text = "", 0, frmWFMEditReportControls.cboTabIndex.Text)
                .Rows(rowIndex).Item("TabName") = frmWFMEditReportControls.cboTabName.Text
                .Rows(rowIndex).Item("UseDefaultProperties") = frmWFMEditReportControls.chkUseDefaultProperties.Checked
                .Rows(rowIndex).Item("UseForMultipleWorkbooks") = frmWFMEditReportControls.chkUseForMultipleWorkbooks.Checked
                .Rows(rowIndex).Item("Width") = IIf(frmWFMEditReportControls.txtWidth.Text = "", 0, frmWFMEditReportControls.txtWidth.Text)
                .Rows(rowIndex).Item("Height") = IIf(frmWFMEditReportControls.txtHeight.Text = "", 0, frmWFMEditReportControls.txtHeight.Text)
                .Rows(rowIndex).Item("LocationX") = IIf(frmWFMEditReportControls.txtLocationX.Text = "", 0, frmWFMEditReportControls.txtLocationX.Text)
                .Rows(rowIndex).Item("LocationY") = IIf(frmWFMEditReportControls.txtLocationY.Text = "", 0, frmWFMEditReportControls.txtLocationY.Text)
                .Rows(rowIndex).Item("ChildControlID") = IIf(frmWFMEditReportControls.cboChildIDs.SelectedValue = "", DBNull.Value, frmWFMEditReportControls.cboChildIDs.SelectedValue)
                .Rows(rowIndex).Item("ContainerID") = IIf(frmWFMEditReportControls.cboContainerID.SelectedValue = "", DBNull.Value, frmWFMEditReportControls.cboContainerID.SelectedValue)
    ...
    Private Sub btnControlDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnControlDelete.Click
        If dgvReportControls.SelectedRows.Count > 0 Then
            ...
            With dgvReportControls.DefaultCellStyle
                'dgvReportControls.SelectedRows.Item(0).DefaultCellStyle.Font = New Font("Tehoma", .Font.Size, FontStyle.Italic)
    ...
    Private Sub btnInsert_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnInsert.Click
        Try
            ...
            If EditingAllowed Then
                UserEdits_TREE = True
                InsertType = IIf(Strings.Left(DirectCast(tvwWFM.SelectedNode, WFMTreeNode).ItemType, 6).ToUpper = "F", "folder", "item")
                If InsertType.ToUpper = "FOLDER" Then
                    If Strings.Left(tvwWFM.SelectedNode.FullPath, 9).ToUpper = "FAVORITES" _
                    Or Strings.Left(tvwWFM.SelectedNode.FullPath, 17).ToUpper = "SCHEDULED REPORTS" Then
                    ...
                    End If
                End If
                frmInsertWFMTreeItem.ShowDialog()
                Dim insertChoice As String = frmInsertWFMTreeItem.Result

                frmInsertWFMTreeItem.Dispose()
                If insertChoice <> "cancel" Then
                    Dim insertOptions() As String = Split(insertChoice, ";")
                    Dim NodeToInsert As New WFMTreeNode

                    Dim InsertAtNode As WFMTreeNode = DirectCast(tvwWFM.SelectedNode, WFMTreeNode)
                    ...
                    Select Case parentIndex
                        Case 0
                            If destItemType = "Folder" Then
                                If folderDropChoice = "in" Then
                                    ...
                                    tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes.Add(NodeToInsert)
                                Else
                                    tvwWFM.Nodes.Insert(InsertAtNode.Index, NodeToInsert)
                                End If
                            Else
                                If folderDropChoice = "before" Then
                                    tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes.Insert(InsertAtNode.Index, NodeToInsert)
                                Else
                                    If InsertAtNode.Index < InsertAtNode.Parent.Nodes.Count - 1 Then
                                        '--- Add 1 to the index to insert after selected node.
                                        tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes.Insert(InsertAtNode.Index + 1, NodeToInsert)
                                    Else
                        ...
                        Case 1
                            If destItemType = "Folder" Then
                                If folderDropChoice = "in" Then
                                    NodeToInsert.TreeLevelID_Parent = InsertAtNode.TreeLevelID
                                    tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes.Add(NodeToInsert)
                                Else
                                    tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes.Insert(InsertAtNode.Index, NodeToInsert)
                                End If
                            Else
                                If folderDropChoice = "before" Then
                                    tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes.Insert(InsertAtNode.Index, NodeToInsert)
                                Else
                                    If InsertAtNode.Index < InsertAtNode.Parent.Nodes.Count - 1 Then
                                        '--- Add 1 to the index to insert after selected node.
                                        tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes.Insert(InsertAtNode.Index + 1, NodeToInsert)
                                    Else
                                        '--- Node at end of list under current parrent.  Therefore, do an ADD. ---
                                        tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes.Add(NodeToInsert)
                                    End If
                        ...
                        Case 2
                            If destItemType = "Folder" Then
                                If folderDropChoice = "in" Then
                                    NodeToInsert.TreeLevelID_Parent = InsertAtNode.TreeLevelID
                                    tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes(CInt(destNodeMap(2))).Nodes.Add(NodeToInsert)
                                Else
                                    tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes.Insert(InsertAtNode.Index, NodeToInsert)
                                End If
                            Else
                                If folderDropChoice = "before" Then
                                    tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes(CInt(destNodeMap(2))).Nodes.Insert(InsertAtNode.Index, NodeToInsert)
                                Else
                                    If InsertAtNode.Index < InsertAtNode.Parent.Nodes.Count - 1 Then
                                        '--- Add 1 to the index to insert after selected node.
                                        tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes(CInt(destNodeMap(2))).Nodes.Insert(InsertAtNode.Index + 1, NodeToInsert)
                                    Else
                                        '--- Node at end of list under current parrent.  Therefore, do an ADD. ---
                                        tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes(CInt(destNodeMap(2))).Nodes.Add(NodeToInsert)
                                    End If
                        ...
                        Case 3
                            If destItemType = "Folder" Then
                                If folderDropChoice = "in" Then
                                    NodeToInsert.TreeLevelID_Parent = InsertAtNode.TreeLevelID
                                    tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes(CInt(destNodeMap(2))).Nodes(CInt(destNodeMap(3))).Nodes.Add(NodeToInsert)
                                Else
                                    tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes(CInt(destNodeMap(2))).Nodes.Insert(InsertAtNode.Index, NodeToInsert)
                                End If
                            Else
                                If folderDropChoice = "before" Then
                                    tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes(CInt(destNodeMap(2))).Nodes(CInt(destNodeMap(3))).Nodes.Insert(InsertAtNode.Index, NodeToInsert)
                                Else
                                    If InsertAtNode.Index < InsertAtNode.Parent.Nodes.Count - 1 Then
                                        '--- Add 1 to the index to insert after selected node.
                                        tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes(CInt(destNodeMap(2))).Nodes(CInt(destNodeMap(3))).Nodes.Insert(InsertAtNode.Index + 1, NodeToInsert)
                                    Else
                                        '--- Node at end of list under current parrent.  Therefore, do an ADD. ---
                                        tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes(CInt(destNodeMap(2))).Nodes(CInt(destNodeMap(3))).Nodes.Add(NodeToInsert)
                                    End If
                        ...
                        Case 4
                            If destItemType = "Folder" Then
                                If folderDropChoice = "in" Then
                                    NodeToInsert.TreeLevelID_Parent = InsertAtNode.TreeLevelID
                                    tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes(CInt(destNodeMap(2))).Nodes(CInt(destNodeMap(3))).Nodes(CInt(destNodeMap(4))).Nodes.Add(NodeToInsert)
                                Else
                                    tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes(CInt(destNodeMap(2))).Nodes(CInt(destNodeMap(3))).Nodes.Insert(InsertAtNode.Index, NodeToInsert)
                                End If
                            Else
                                If folderDropChoice = "before" Then
                                    tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes(CInt(destNodeMap(2))).Nodes(CInt(destNodeMap(3))).Nodes(CInt(destNodeMap(4))).Nodes.Insert(InsertAtNode.Index, NodeToInsert)
                                Else
                                    If InsertAtNode.Index < InsertAtNode.Parent.Nodes.Count - 1 Then
                                        '--- Add 1 to the index to insert after selected node.
                                        tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes(CInt(destNodeMap(2))).Nodes(CInt(destNodeMap(3))).Nodes(CInt(destNodeMap(4))).Nodes.Insert(InsertAtNode.Index + 1, NodeToInsert)
                                    Else
                                        '--- Node at end of list under current parrent.  Therefore, do an ADD. ---
                                        tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes(CInt(destNodeMap(2))).Nodes(CInt(destNodeMap(3))).Nodes(CInt(destNodeMap(4))).Nodes.Add(NodeToInsert)
                                    End If
                        ...
                        Case 5
                            If destItemType = "Folder" Then
                                If folderDropChoice = "in" Then
                                    NodeToInsert.TreeLevelID_Parent = InsertAtNode.TreeLevelID
                                    tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes(CInt(destNodeMap(2))).Nodes(CInt(destNodeMap(3))).Nodes(CInt(destNodeMap(4))).Nodes(CInt(destNodeMap(5))).Nodes.Add(NodeToInsert)
                                Else
                                    tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes(CInt(destNodeMap(2))).Nodes(CInt(destNodeMap(3))).Nodes(CInt(destNodeMap(4))).Nodes.Insert(InsertAtNode.Index, NodeToInsert)
                                End If
                            Else
                                If folderDropChoice = "before" Then
                                    tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes(CInt(destNodeMap(2))).Nodes(CInt(destNodeMap(3))).Nodes(CInt(destNodeMap(4))).Nodes(CInt(destNodeMap(5))).Nodes.Insert(InsertAtNode.Index, NodeToInsert)
                                Else
                                    If InsertAtNode.Index < InsertAtNode.Parent.Nodes.Count - 1 Then
                                        '--- Add 1 to the index to insert after selected node.
                                        tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes(CInt(destNodeMap(2))).Nodes(CInt(destNodeMap(3))).Nodes(CInt(destNodeMap(4))).Nodes(CInt(destNodeMap(5))).Nodes.Insert(InsertAtNode.Index + 1, NodeToInsert)
                                    Else
                                        '--- Node at end of list under current parrent.  Therefore, do an ADD. ---
                                        tvwWFM.Nodes(CInt(destNodeMap(0))).Nodes(CInt(destNodeMap(1))).Nodes(CInt(destNodeMap(2))).Nodes(CInt(destNodeMap(3))).Nodes(CInt(destNodeMap(4))).Nodes(CInt(destNodeMap(5))).Nodes.Add(NodeToInsert)
                                    End If
                    ...
                    tvwWFM.SelectedNode = NodeToInsert
                    ...
                    rbReport.Checked = True
                    editmode = adding
                    HandleNodeClick(tvwWFM.SelectedNode)
                    If insertOptions(0) = "paste" Then
    ...
    Private Sub rb_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rbReport.CheckedChanged, rbFolder.CheckedChanged
        If rbReport.Checked = True Then
            SetItemTypeSelectors("R")
            DirectCast(tvwWFM.SelectedNode, WFMTreeNode).ItemType = "R"
            BuildItemTypeSelector("Reports", "IdName", "ReportID")
        Else
            SetItemTypeSelectors("F")

            DirectCast(tvwWFM.SelectedNode, WFMTreeNode).ItemType = "F"
            BuildItemTypeSelector("Folders", "IdName", "FolderID")
        End If
        DisplayProperties(tvwWFM.SelectedNode)
    ...
    Private Sub tvwWFM_DragDrop(ByVal sender As Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles tvwWFM.DragDrop
        CheckEditingStatus()
        If EditingAllowed Then
            SetTreeDataState("dirty")

            Dim NewNode As WFMTreeNode
            Dim ParentSPID As Integer = 0
            Dim DestinationNode As WFMTreeNode
            ...
            If DestinationNode.NodeType = DDT_NodeType.Folder Then
                ParentSPID = Dest
            Else
                ParentSPID = CType(DestinationNode.Parent, WFMTreeNode).SpecialID
                If DestinationNode.PrevNode Is Nothing Then
                    PrevID = 0
                Else
                    PrevID = CType(DestinationNode.PrevNode, WFMTreeNode).SpecialID
                End If
            End If
    ...
    Public Sub EraseSelectionRectangle()
        Dim g As Graphics = tvwWFM.CreateGraphics()
        Dim erasePen As New Pen(Color.White)
        erasePen.DashStyle = Drawing2D.DashStyle.Dot
        g.DrawRectangle(erasePen, lastInsertRect)
    End Sub

    Private Sub tvwWFM_DragOver1(ByVal sender As Object, ByVal e As System.Windows.Forms.DragEventArgs) _
    Handles tvwWFM.DragOver
        Try
            ...
            If TypeOf sender Is TreeView Then
                ...
                Dim dropNode As WFMTreeNode = DirectCast(tv.GetNodeAt(position), WFMTreeNode)
    ...
    Private Sub tvwWFM_DragOver(ByVal sender As Object, ByVal e As System.Windows.Forms.DragEventArgs)
        Dim gr As Graphics = tvwWFM.CreateGraphics
        ...
        If TypeOf sender Is TreeView Then
            ...
            position = tv.PointToClient(position)
            Dim dropNode As WFMTreeNode = DirectCast(tv.GetNodeAt(position), WFMTreeNode)
            Dim DraggedNode As WFMTreeNode = tv.SelectedNode
    ...
    Public Sub tvwWFM_ItemDrag(ByVal sender As System.Object, ByVal e As System.Windows.Forms.ItemDragEventArgs) _
    Handles tvwWFM.ItemDrag, tvwWFM.ItemDrag
        'Set the drag node and initiate the DragDrop 
        tvwWFM.DoDragDrop(e.Item, DragDropEffects.Move)
    End Sub
    ...
    Private Sub tvwWFM_MouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles tvwWFM.MouseDown, tvwWFM.MouseDown
        DraggedNode = DirectCast(tvwWFM.GetNodeAt(e.X, e.Y), WFMTreeNode)
        tvwWFM.SelectedNode = DraggedNode
    End Sub
    ...
    Private Sub MoveNode(ByVal direction As String)
        Try
            Dim thisNode As WFMTreeNode = tvwWFM.SelectedNode
            Dim thisNodeParent As WFMTreeNode = tvwWFM.SelectedNode.Parent
            ...
            Select Case direction.ToUpper
                ...
                Case "DOWN"
                    offset = 1
            End Select
            If tvwWFM.SelectedNode.Parent.Level <> Nothing Then
                Dim parentNodeLevel As Integer = tvwWFM.SelectedNode.Parent.Level
            End If

            tvwWFM.SelectedNode.Remove()
            thisNodeParent.Nodes.Insert(thisNode.Index + offset, thisNode)
            tvwWFM.SelectedNode = thisNode
            tvwWFM.SelectedNode.EnsureVisible()
            tvwWFM.Focus()
            HandleNodeClick(tvwWFM.SelectedNode)
            HandleControls()
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        Try
            Dim SqlStr As String = "dbo.sToolbox_UpdateTreeLevels_Delete " + DirectCast(tvwWFM.SelectedNode, WFMTreeNode).TreeLevelID.ToString + ", " + EmpNumber.ToString
            Dim result As String
            Dim tmpCurrNode As WFMTreeNode = DirectCast(tvwWFM.SelectedNode, WFMTreeNode)
            Dim tmpPrevNode As WFMTreeNode = tmpCurrNode.PrevNode
            Dim tmpNextNode As WFMTreeNode = tmpCurrNode.NextNode
            Dim tmpParentNode As WFMTreeNode = tmpCurrNode.Parent
            ...
            If EditingAllowed Then
                If SelectedNodeHasChildren Then
                    ...
                    Exit Sub
                End If
                If RichMessageBoxShow("Delete Node < [ID=" + tmpCurrNode.TreeLevelID.ToString + "]  " + tvwWFM.SelectedNode.Text + ">  ", _
                ...
                = Windows.Forms.DialogResult.OK Then
                    ...
                    If tmpPrevNode IsNot Nothing Then
                        Call WFMGetNodeByKey(tvwWFM, "TreeLevelID=" + DirectCast(tmpPrevNode, WFMTreeNode).TreeLevelID.ToString, True)
                        HandleNodeClick(DirectCast(tmpPrevNode, WFMTreeNode))
                    ElseIf tmpNextNode IsNot Nothing Then
                        Call WFMGetNodeByKey(tvwWFM, "TreeLevelID=" + DirectCast(tmpNextNode, WFMTreeNode).TreeLevelID.ToString, True)
                        HandleNodeClick(DirectCast(tmpNextNode, WFMTreeNode))
                    Else
                        Call WFMGetNodeByKey(tvwWFM, "TreeLevelID=" + DirectCast(tmpParentNode, WFMTreeNode).TreeLevelID.ToString, True)
                        HandleNodeClick(DirectCast(tmpParentNode, WFMTreeNode))
                    End If
                    HandleControls()
                Else
                End If
    ...
    Handles chkActive.CheckedChanged, chkSchedulingEnabled.CheckedChanged, chkHelpButton.CheckedChanged
        ...
        Select Case sender.name
            Case "chkSchedulingEnabled"
                If chkSchedulingEnabled.Checked <> DirectCast(tvwWFM.SelectedNode, WFMTreeNode).ScheduleEnabled Then
                    UserEdits_TREE = True
                End If
            Case "chkHelpButton"
                If chkHelpButton.Checked <> DirectCast(tvwWFM.SelectedNode, WFMTreeNode).HelpEnabled Then
                    UserEdits_TREE = True
                End If
            Case "chkActiveButton"
                If chkActive.Checked <> DirectCast(tvwWFM.SelectedNode, WFMTreeNode).IsActive Then
                    UserEdits_TREE = True
                End If
        End Select
...
#End Region
        ...
        For Each dgvr As DataGridViewRow In dgvReportControls.Rows
            ...
            Select Case ControlName
                ...
                Case "DDT_ListBox"
                    ...
                    obj.Text = ControlName
                    obj.ControlTitle = dgvr.Cells("ControlTitle").Value
                Case "DDT_DataGridView"
                    obj = New DDT_DataGridView
                    obj.Text = ControlName
                    obj.ControlTitle = dgvr.Cells("ControlTitle").Value
                Case "DDT_DataGridView2"
                    obj = New DDT_DataGridView
                    obj.Text = ControlName
                    obj.ControlTitle = dgvr.Cells("ControlTitle").Value
    ...
    Private Sub AddTabs()
        ...
        For Each dr As DataGridViewRow In dgvReportControls.Rows
            If IIf(IsDBNull(dr.Cells("Tabname").Value), "", dr.Cells("Tabname").Value.ToString) <> currTabname.ToString Then
                ...
                '--- Draw message label and line showing default Right Margin for WFM ---
    ...
    Private Sub cboItemTypeSelect_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles cboItemTypeSelect.SelectedValueChanged
        If cboItemTypeSelect.SelectedValue IsNot Nothing Then
            If EditingAllowed = True And editmode = adding And cboItemTypeSelect.SelectedValue.ToString <> "System.Data.DataRowView" Then
                'mburts
                If DirectCast(tvwWFM.SelectedNode, WFMTreeNode).ItemType = "F" Then
    ...
    Public Function GetTreeNodeDetails(ByVal rowItem As DataRow) As WFMTreeNode
        CallFromType = "Function"
        CallFrom = "GetTreeNodeDetails"
    ...
    Public Function WFMGetNodeByKey(ByVal aTreeView As TreeView, ByVal FindKey As String, _
...
Optional ByRef arrNodes As TreeNodeCollection = Nothing) As WFMTreeNode
        ...
        Try
            ...
            For Each aNode In arrNodes
                ...
                End Select
                If aNode.ItemType = "F" And aNode.Nodes.Count > 0 Then
                    Dim tmpNode As WFMTreeNode = WFMGetNodeByKey(aTreeView, FindKey, DoSelect, aNode.Nodes)
